...
namespace DotnetBleServer.Core
{
    public class BlueZManager
    {
        ...
        public BlueZManager(ServerContext context)
        {
            _context = context;
        }
        ...
        public async Task<IAdapter1> GetAdapterAsync(string adapterName, bool fullName = false)
        {
            ...
            var adapter = _context.Connection.CreateProxy<IAdapter1>(Constants.DbusServicePath, adapterObjectPath);
        ...
        internal async Task<IReadOnlyList<T>> GetProxiesAsync<T>(string interfaceName, IDBusObject rootObject)
        {
            ...
            var objectManager = _context.Connection.CreateProxy<IObjectManager>(Constants.DbusServicePath, "/");
            ...
            var proxies = matchingObjectPaths
                .Select(objectPath => _context.Connection.CreateProxy<T>(Constants.DbusServicePath, objectPath))
                .ToList();
